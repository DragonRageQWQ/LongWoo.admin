...
export default function OrderDetailModal({
  ...
  // 数据是否已变更（关闭时用于通知父组件刷新）
  const [dataChanged, setDataChanged] = useState(false);

  // 刷新触发器：操作后重新获取详情
  const [refreshKey, setRefreshKey] = useState(0);

  // 数据获取：所有 setState 都在 await 之后
  useEffect(() => {
    let cancelled = false;
  ...
  }, [enlargedImage, onClose, dataChanged]);
                      ...
                      <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                        {order.attachments.map((att) => {
                          ...
                          return (
                            <div
                              key={att.id}
                              className="group relative aspect-square bg-white rounded-lg border border-gray-200 overflow-hidden cursor-pointer hover:border-lw-accent transition-colors"
                              onClick={() =>
