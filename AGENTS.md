...
> 决策：**放弃旧首页优化，忠于新界面**（2026-09-04，站点负责人确认）

- **新界面（正主）**：官网新首页 `src/components/new-home/GrayTest2App.tsx`（线上 `/`，纸墨极简 Ink on Paper）及其同风格功能页（`/sampler`、`/profile` 等）。后续功能开发一律以新界面为准。
- **旧首页（已冻结）**：归档于 `public/gray-test/test2/index.html`（原旧版整页首页，`/index.html` 已 301 至新首页，页面含 noindex）。
- 冻结规则：对旧首页（含其静态页面体系与相关旧式视觉）**不再进行任何功能优化**，仅允许两类改动：
  1. 线上安全/合规修复；
  2. 使其继续指向新界面的兼容重定向。
- 维护者与 AI 助手在接到涉及旧首页/旧视觉的优化请求时，应提示负责人改投新界面方案，除非负责人明确豁免。
